...
"""
#
#There are two primary ways to utilize this module, either through providing and
...
def MCMC_step_vertex(A,edge_list,swaps,allow_loops,allow_multi):
    ...
    |     edge_list (n-sized list): List of edges in A. Node names should be 
...
class MCMC_class:
    ...
    def __init__(self,G,allow_loops,allow_multi,is_v_labeled = True):
        ...
        self.A += np.diag(np.diag(self.A)) # This is done so that a self-loop (u,u) contributes 2 to the diagonal of the adjacency matrix, and not 1 as in Networkx.
        #List_edges = self.G.edges()
        List_edges = List()
        [List_edges.append(e) for e in self.G.edges()] # numba throws warning if we try to use List_edges = self.G.edges() instead. So we use typed.List here.
        self.edge_list = List_edges # This is of type List, not numpy array (changed due to Python2 -> Python3)
...
def flatten_graph(graph,allow_loops,allow_multi):
    ...
    # You reach here if you are not allowing self-loops. So you have to remove all the existing self loops of the provided Graph.
