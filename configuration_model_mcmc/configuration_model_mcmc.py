...
def graphs_after_McmcConvergence(G, spacing, allow_loops, allow_multi, is_vertex_labeled, count):
    '''
    Input --------------------
    ...
    '''
    #print("Sampling networks from the Configuration model.")
    ...
    for i in range(count-1):
        ...
        graphs_from_configModel.append(newgraph)
    if count == 1:
        return graphs_from_configModel[0]
    return graphs_from_configModel   
...
def convert_graph(G, allow_loops, allow_multi, is_vertex_labeled):
    ...
    if nx.is_weighted(G):
        print("Cannot apply double edge swap to weighted networks.")
        return False
    if G.is_directed():
        print("Cannot apply double edge swap to directed networks.")
        return False
...
def run_sampling_gap_algorithm(base_sampling_gap, G, allow_loops, allow_multi, is_vertex_labeled):
    ...
    graph_of_graphs = mcmc.MCMC_class(G, allow_loops, allow_multi, is_vertex_labeled)
    for j in range(1000*m): # Burn-in period
        G2, swap_list = graph_of_graphs.step_and_get_graph()
    
    for i in range(100): # Initialising 100 MCMC walks all starting from G2 obatined after burn-in
        graph_of_graphs = mcmc.MCMC_class(G2, allow_loops, allow_multi, is_vertex_labeled)
    ...
    #print("Sampling gap calculated")
    return int((eta0_200+eta0_250)/2)

def get_sampling_gap(G, allow_loops, allow_multi, is_vertex_labeled):
    m = G.number_of_edges()
    if allow_multi == False:
        density_criterion_satisfied = check_density_criterion(G, allow_loops)
        if density_criterion_satisfied == 1 or m <= 1000:
            ...
            sampling_gap = run_sampling_gap_algorithm(int(m/3 + 300), G, allow_loops, allow_multi, is_vertex_labeled)
        ...
        if is_vertex_labeled == False: 
            ...
            maxDegree_criterion_satisfied = check_maxDegree_criterion(G)
            if maxDegree_criterion_satisfied == 1 or m <= 1000:
                ...
                sampling_gap = run_sampling_gap_algorithm(int(m/2.7 + 100), G, allow_loops, allow_multi, is_vertex_labeled)
                #print("Sampling gap of the network = ", sampling_gap)
                return sampling_gap
            
def get_graphs_from_ConfigModel(G, allow_loops, allow_multi, is_vertex_labeled, count=1):
    '''
    Input --------------------
    ...
    '''
    if allow_loops == True and allow_multi == False: # Loopy graph space
        G_degree = list(nx.degree(G))
        degreeList = []
        for eachpair in G_degree:
            degreeList.append(eachpair[1])
        if is_disconnected(degreeList) == True:
            print("The loopy graph space for this degree sequence is disconnected. Exiting.")
            return False
    
    returned = convert_graph(G, allow_loops, allow_multi, is_vertex_labeled)
    if returned == False:
        return False # G is a weighted or a directed network
    else:
        converted_G = returned[0]
        hash_map = returned[1]
        reverse_hash_map  = returned[2]
    spacing = get_sampling_gap(converted_G, allow_loops, allow_multi, is_vertex_labeled)
    list_of_networks = graphs_after_McmcConvergence(converted_G, spacing, allow_loops, allow_multi, is_vertex_labeled, count)
    networks_from_configModel = []
    for each_net in list_of_networks:
        reverseHashed_net = revert_back(each_net, reverse_hash_map, allow_loops, allow_multi, is_vertex_labeled)
